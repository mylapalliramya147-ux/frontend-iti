...
<head>
    ...
    <title>ITI Student List Report</title>
...
<body class="nodal-body">
    ...
    <div class="nodal-page-title-dashboard">
        <h2>Industrial Training Institute — ITI Student List Report</h2>
    </div>
    ...
    <div class="loader-spinner" id="loader">
        ...
        <p class="mt-3 fw-bold">Processing student data from database...</p>
    </div>
    ...
    <div class="container-fluid px-4 py-4" id="reportView" style="display: none;">
        ...
        <div class="text-center mb-3" style="color: #003366;">
            <h2 class="fw-bold fs-4 mb-2" id="reportTitle">ITI Student List Report</h2>
            <p class="mb-4 text-muted" style="font-size: 1.1rem;" id="reportTotal">Total Students: 0</p>
        </div>
        ...
        <div class="no-print d-flex justify-content-center gap-3 mb-5">
            ...
            <button class="btn btn-dark shadow-sm px-4 rounded-pill fw-bold" onclick="exportTableToExcel('studentTable', 'Student_List_Report')">
        ...
        <div class="shadow" style="background-color: #fff; border-radius: 8px; overflow: hidden; border: 1px solid #e0e0e0;">
            <div style="overflow-y: auto; max-height: 600px;">
                <table class="table table-bordered mb-0 table-hover text-center report-table" id="studentTable" style="min-width: 1500px;">
                    <thead>
                        <tr>
                            <th>SNO</th>
                            <th>ADMISSION NO</th>
    ...
    <script>
        ...
        function fetchReport(event) {
            ...
            fetch('http://10.72.4.135:5051/reports/iti-student-list', { credentials: 'include',
                method: 'POST',
            ...
            .then(data => {
                ...
                document.getElementById('reportTitle').innerText = 'ITI Student List Report (' + year + ')';
                ...
                if (data.students && data.students.length > 0) {
                    document.getElementById('reportTotal').innerText = 'Total Students: ' + data.total_students;
                    data.students.forEach((student, index) => {
                        const tr = document.createElement('tr');
                        tr.innerHTML = `
                            <td style="font-weight: 600;">\${index + 1}</td>
                            <td style="font-weight: 700;">\${student['Admission No'] || ''}</td>
                            <td>\${student['SSC HallTicket'] || ''}</td>
                            <td style="text-align: left; font-weight: 600;">\${student['Name of Candidate'] || ''}</td>
                            <td style="text-align: left;">\${student['Father Name'] || ''}</td>
                            <td style="text-align: left;">\${student['Mother Name'] || ''}</td>
                            <td>\${student['Date of Birth'] || ''}</td>
                            <td>\${student['Mobile No'] || ''}</td>
                            <td>\${student['Email'] || ''}</td>
                            <td>\${student['Shift'] || ''}</td>
                            <td>\${student['Unit'] || ''}</td>
                            <td>\${student['PWD category'] || ''}</td>
                            <td>\${student['Economic Weaker Section'] || ''}</td>
                            <td>\${student['Is Trainee Dual Mode'] || ''}</td>
                        `;
                        tbody.appendChild(tr);
                    });
                } else {
                    document.getElementById('reportTotal').innerText = 'Total Students: 0';
                    tbody.innerHTML = '<tr><td colspan="14" style="text-align:center; padding:20px; font-weight: bold;">No students found matching your criteria.</td></tr>';
                }
            })
            .catch(error => {
                document.getElementById('loader').style.display = 'none';
