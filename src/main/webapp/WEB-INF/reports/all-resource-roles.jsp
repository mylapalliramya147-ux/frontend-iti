...
<body class="nodal-body">
    ...
    <div class="nodal-page-title-dashboard"><h2>All Resource Roles Report</h2></div>
        ...
        <div class="shadow" style="background-color: #fff; border-radius: 8px; overflow: hidden; border: 1px solid #e0e0e0;">
            <div style="overflow-y: auto; max-height: 600px;">
                <table class="table table-bordered mb-0 table-hover text-center report-table" id="statusTable" style="min-width: 800px;">
                    <thead><tr><th>District</th><th style="text-align: left;">ITI Name</th><th>Trade</th><th>Type</th><th class="num">Strength</th><th class="num">Fill</th><th class="num">Vacant</th></tr></thead>
            ...
            .then(data => {
                ...
                if (data.data && data.data.length > 0) {
                    data.data.forEach(row => {
                        ...
                        tr.innerHTML = '<td>' + (row.distName || '-') + '</td><td style="text-align: left;">' + (row.itiName || '-') + '</td><td style="text-align: left;">' + (row.tradeName || '-') + '</td><td>' + (row.type || '-') + '</td><td class="num">' + (row.strength || 0) + '</td><td class="num text-primary">' + (row.fill || 0) + '</td><td class="num text-success">' + (row.vacant || 0) + '</td>';
