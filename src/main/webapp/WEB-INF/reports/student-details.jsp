...
<body class="nodal-body">
    ...
    <div class="nodal-page-title-dashboard"><h2>Student Details Report</h2></div>
            ...
            <div class="p-5 bg-white rounded-bottom">
                <form id="reportForm" onsubmit="fetchReport(event)">
                    ...
                    <div class="row align-items-center mb-4">
                        ...
                        <div class="col-md-7"><input type="text" name="itiCode" id="itiCode" class="form-control-official" placeholder="Enter ITI Code"></div>
                    </div>
    ...
    <div class="container-fluid px-4 py-4" id="reportView" style="display: none;">
        <div class="text-center mb-3" style="color: #003366;"><h2 class="fw-bold fs-4 mb-2" id="reportTitle">Student Details Report</h2></div>
        ...
        <div class="shadow" style="background-color: #fff; border-radius: 8px; overflow: hidden; border: 1px solid #e0e0e0;">
            <div style="overflow-y: auto; max-height: 600px;">
                <table class="table table-bordered mb-0 table-hover text-center report-table" id="statusTable" style="min-width: 1000px;">
                    <thead><tr><th>S.No</th><th>Name</th><th>ITI Code</th><th>ITI Name</th><th>District</th><th>Trade</th><th>Parent/Guardian Name</th><th>Mobile</th><th>Aadhaar</th></tr></thead>
    ...
    <script>
        ...
        function fetchReport(event) {
            ...
            if (itiCode) params += '&itiCode=' + encodeURIComponent(itiCode);
            fetch('${backendApiUrl}/student-details?' + params, { method: 'GET' })
            .then(response => response.json())
            .then(data => {
                document.getElementById('loader').style.display = 'none';
                document.getElementById('reportView').style.display = 'block';
                document.getElementById('reportTitle').innerText = 'Student Details Report (' + year + ')';
                ...
                if (data.data && data.data.length > 0) {
                    data.data.forEach((row, idx) => {
                        ...
                        tr.innerHTML = '<td class="num">' + (idx + 1) + '</td><td style="text-align: left;">' + (row.studentName || '-') + '</td><td>' + (row.itiCode || '-') + '</td><td style="text-align: left;">' + (row.itiName || '-') + '</td><td>' + (row.distName || '-') + '</td><td style="text-align: left;">' + (row.tradeName || '-') + '</td><td style="text-align: left;">' + (row.parentName || '-') + '</td><td>' + (row.mobile || '-') + '</td><td>' + (row.aadhaar || '-') + '</td>';
                        tbody.appendChild(tr);
                    });
                } else { tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; padding:20px; font-weight: bold;">No records found.</td></tr>'; }
            })
