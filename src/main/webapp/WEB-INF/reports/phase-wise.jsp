...
<head>
    ...
    <title>Phase-wise Admissions | Nodal Reports</title>
...
<body class="nodal-body">
    ...
    <div class="nodal-page-title-dashboard"><h2>Phase-wise Admissions Details</h2></div>
    ...
    <div class="container mt-4" id="reportView" style="display: none;">
        <div class="text-center mb-3" style="color: #003366;"><h2 class="fw-bold fs-4 mb-2" id="reportTitle">Phase-wise Admissions Details</h2></div>
        ...
        <div class="shadow" style="background-color: #fff; border-radius: 8px; overflow: hidden; border: 1px solid #e0e0e0;">
            <div style="overflow-y: auto; max-height: 600px;">
                <table class="table table-bordered mb-0 table-hover text-center report-table" id="statusTable" style="min-width: 1000px;">
                    <thead><tr><th>District Name</th><th class="num">Phase I</th><th class="num">Phase II</th><th class="num">Phase III</th><th class="num">Phase IV</th><th class="num">Phase V</th><th class="num">Total</th><th class="num">Today</th></tr></thead>
            ...
            .then(data => {
                ...
                if (data.data && data.data.length > 0) {
                    data.data.forEach(row => {
                        ...
                        tr.innerHTML = '<td style="text-align: left;">' + (row.distName || '-') + '</td><td class="num">' + (row.phaseI || 0) + '</td><td class="num">' + (row.phaseII || 0) + '</td><td class="num">' + (row.phaseIII || 0) + '</td><td class="num">' + (row.phaseIV || 0) + '</td><td class="num">' + (row.phaseV || 0) + '</td><td class="num">' + (row.total || 0) + '</td><td class="num">' + (row.today || 0) + '</td>';
                        tbody.appendChild(tr);
                        totals.phaseI += row.phaseI || 0; totals.phaseII += row.phaseII || 0; totals.phaseIII += row.phaseIII || 0; totals.phaseIV += row.phaseIV || 0; totals.phaseV += row.phaseV || 0; totals.total += row.total || 0; totals.today += row.today || 0;
                    });
                    const ft = document.createElement('tr'); ft.className = 'total-row';
                    ft.innerHTML = '<td style="text-align: right; padding-right: 30px; font-weight: bold;">GRAND TOTAL</td><td class="num">' + totals.phaseI + '</td><td class="num">' + totals.phaseII + '</td><td class="num">' + totals.phaseIII + '</td><td class="num">' + totals.phaseIV + '</td><td class="num">' + totals.phaseV + '</td><td class="num">' + totals.total + '</td><td class="num">' + totals.today + '</td>';
