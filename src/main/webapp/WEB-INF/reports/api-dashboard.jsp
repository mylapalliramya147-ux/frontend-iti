...
<head>
    ...
    <title>API Dashboard | District Reports</title>
...
<body class="nodal-body">
    ...
    <div class="nodal-page-title-dashboard"><h2>API Dashboard (District)</h2></div>
    ...
    <div class="loader-spinner" id="loader"><i class="fas fa-spinner fa-spin fa-3x"></i><p class="mt-3 fw-bold">Loading API dashboard...</p></div>
    <div class="container-fluid px-4 py-4" id="reportView" style="display: none;">
        <div class="text-center mb-3" style="color: #003366;"><h2 class="fw-bold fs-4 mb-2" id="reportTitle">API Dashboard (District)</h2></div>
        ...
        <div class="shadow" style="background-color: #fff; border-radius: 8px; overflow: hidden; border: 1px solid #e0e0e0;">
            <div style="overflow-y: auto; max-height: 600px;">
                <table class="table table-bordered mb-0 table-hover text-center report-table" id="statusTable" style="min-width: 1400px;">
                    <thead><tr><th>District Name</th><th class="num">Total</th><th class="num">Success</th><th class="num">Pending (SID)</th><th class="num">Verified</th><th class="num">To Be Verified</th><th class="num">To Be Updated</th><th class="num">Phone Dupes</th><th class="num">Email Dupes</th><th class="num">Aadhar Dupes</th></tr></thead>
    ...
    <script>
        ...
        function fetchReport(event) {
            ...
            fetch('${backendApiUrl}/api-dashboard?year=' + encodeURIComponent(year), { method: 'GET' })
            .then(response => response.json())
            .then(data => {
                document.getElementById('loader').style.display = 'none';
                document.getElementById('reportView').style.display = 'block';
                document.getElementById('reportTitle').innerText = 'API Dashboard - Year ' + year;
                ...
                if (data.data && data.data.length > 0) {
                    data.data.forEach(row => {
                        ...
                        tr.innerHTML = '<td style="text-align: left;">' + (row.distName || '-') + '</td><td class="num">' + (row.total || 0) + '</td><td class="num text-success">' + (row.success || 0) + '</td><td class="num text-warning">' + (row.pendingSid || 0) + '</td><td class="num text-primary">' + (row.verified || 0) + '</td><td class="num text-danger">' + (row.toBeVerified || 0) + '</td><td class="num">' + (row.toBeUpdated || 0) + '</td><td class="num text-danger">' + (row.phoneDuplicateRecords || 0) + '</td><td class="num text-danger">' + (row.emailDuplicateRecords || 0) + '</td><td class="num text-danger">' + (row.aadharDuplicateRecords || 0) + '</td>';
                    ...
                    ft.innerHTML = '<td style="text-align: right; padding-right: 30px; font-weight: bold;">GRAND TOTAL</td><td class="num">' + totals.total + '</td><td class="num">' + totals.success + '</td><td class="num">' + totals.pendingSid + '</td><td class="num">' + totals.verified + '</td><td class="num">' + totals.toBeVerified + '</td><td class="num">' + totals.toBeUpdated + '</td><td class="num">' + totals.phone + '</td><td class="num">' + totals.email + '</td><td class="num">' + totals.aadhar + '</td>';
