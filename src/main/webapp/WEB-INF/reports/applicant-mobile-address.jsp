...
<head>
    ...
    <title>Applicant Mobile Address Report | District Reports</title>
...
<body class="nodal-body">
    ...
    <div class="loader-spinner" id="loader"><i class="fas fa-spinner fa-spin fa-3x"></i><p class="mt-3 fw-bold">Loading applicant address data...</p></div>
    <div class="container mt-4" id="reportView" style="display: none;">
        <div class="text-center mb-3" style="color: #003366;"><h2 class="fw-bold fs-4 mb-2" id="reportTitle">Applicant Address With Mobile</h2></div>
        ...
        <div class="shadow" style="background-color: #fff; border-radius: 8px; overflow: hidden; border: 1px solid #e0e0e0;">
            <div style="overflow-y: auto; max-height: 600px;">
                <table class="table table-bordered mb-0 table-hover text-center report-table" id="statusTable" style="min-width: 1000px;">
                    <thead><tr><th>Reg ID</th><th>Name</th><th>Father Name</th><th>Mobile</th><th>Address</th><th>District</th></tr></thead>
    ...
    <script>
        ...
        function fetchReport(event) {
            ...
            fetch('${backendApiUrl}/applicant-mobile-address?year=' + encodeURIComponent(year) + '&distCode=All&page=0&size=10000', {
                method: 'GET'
            })
            ...
            .then(data => {
                ...
                document.getElementById('reportTitle').innerText = 'Applicant Address With Mobile (' + year + ')';
                ...
                if (data.data && data.data.length > 0) {
                    data.data.forEach(row => {
                        ...
                        tr.innerHTML = '<td>' + (row.regId || '-') + '</td><td style="text-align: left;">' + (row.name || '-') + '</td><td style="text-align: left;">' + (row.fatherName || '-') + '</td><td>' + (row.mobile || '-') + '</td><td style="text-align: left;">' + (row.address || '-') + '</td><td>' + (row.distName || '-') + '</td>';
