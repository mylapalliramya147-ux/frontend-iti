...
<head>
    ...
    <title>Admission Report (ITI) | ITI Reports</title>
...
<body class="nodal-body">
    ...
    <div class="nodal-page-title-dashboard"><h2>ITI Admission Report</h2></div>
        ...
        <div class="nodal-report-card shadow-lg" style="max-width: 550px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 12px;">
            ...
            <div class="p-5 bg-white rounded-bottom">
                <form id="reportForm" onsubmit="fetchReport(event)">
                    ...
                    <div class="row align-items-center mb-4">
                        <div class="col-md-5"><label for="caste" class="form-label-official mb-md-0">Caste Category</label></div>
                        <div class="col-md-7">
                            <select name="caste" id="caste" class="form-select-official w-100">
                                <option value="All">All Categories</option><option value="BC-A">BC-A</option><option value="BC-B">BC-B</option><option value="BC-C">BC-C</option><option value="BC-D">BC-D</option><option value="BC-E">BC-E</option><option value="OC">OC</option><option value="SC">SC</option><option value="ST">ST</option><option value="EWS">EWS</option>
                            </select>
                        </div>
                    </div>
                    <div class="row align-items-center mb-4">
                        <div class="col-md-5"><label for="pwd" class="form-label-official mb-md-0">PWD</label></div>
                        <div class="col-md-7">
                            <select name="pwd" id="pwd" class="form-select-official w-100">
    ...
    <div class="loader-spinner" id="loader"><i class="fas fa-spinner fa-spin fa-3x"></i><p class="mt-3 fw-bold">Loading admission report...</p></div>
    <div class="container-fluid px-4 py-4" id="reportView" style="display: none;">
        <div class="text-center mb-3" style="color: #003366;"><h2 class="fw-bold fs-4 mb-2" id="reportTitle">ITI Admission Report</h2></div>
        ...
        <div class="shadow" style="background-color: #fff; border-radius: 8px; overflow: hidden; border: 1px solid #e0e0e0;">
            <div style="overflow-y: auto; max-height: 600px;">
                <table class="table table-bordered mb-0 table-hover text-center report-table" id="statusTable" style="min-width: 600px;">
                    <thead><tr><th>TRADE NAME</th><th class="num">BOYS</th><th class="num">GIRLS</th><th class="num">TOTAL</th></tr></thead>
    ...
    <script>
        ...
        function fetchReport(event) {
            ...
            fetch('${backendApiUrl}/admission-report-iti?year=' + encodeURIComponent(year) + '&caste=' + encodeURIComponent(caste) + '&pwd=' + encodeURIComponent(pwd), {
                method: 'GET'
            })
            ...
            .then(data => {
                ...
                document.getElementById('reportTitle').innerText = 'ITI Admission Report (' + year + ')';
                ...
                if (data.data && data.data.length > 0) {
                    data.data.forEach(row => {
                        ...
                        tr.innerHTML = '<td style="text-align: left;">' + (row.tradeName || '-') + '</td><td class="num text-primary">' + (row.boys || 0) + '</td><td class="num text-danger">' + (row.girls || 0) + '</td><td class="num fw-bold">' + (row.total || 0) + '</td>';
                        tbody.appendChild(tr);
                        totalBoys += row.boys || 0; totalGirls += row.girls || 0; totalAll += row.total || 0;
                    });
                    const ft = document.createElement('tr'); ft.className = 'total-row';
                    ft.innerHTML = '<td style="text-align: right; padding-right: 30px; font-weight: bold;">GRAND TOTAL</td><td class="num">' + totalBoys + '</td><td class="num">' + totalGirls + '</td><td class="num">' + totalAll + '</td>';
